<?php
#####################################################
#          FICHIER DE GESTION DES RECENSEMENTS      #
#####################################################
session_start();
 ...
 if(isset($_GET['campagneID']))
 {
    //Sélection d'édition de la campagne et de la commune
    ...
    $req->execute([$_SESSION['id'],htmlspecialchars($_GET['campagneID'])]);
    $data0=$req->fetch();
...
while(isMatriculeExist($bdd,$matricule))
{
 ...
 if(isset($_POST['campagneid']))
 {
    ...
    $req=$bdd->prepare('SELECT id_comcam FROM association_cuc WHERE id_user=? AND id_campagne=?');
    $req->execute([$_SESSION['id'],htmlspecialchars($_POST['campagneid'])]);
   $commune="";
...
while(isMatriculeExist($bdd,$matricule))
{
 ...
 {  
    ...
    //Inserstion de la femme 
      $req=$bdd->prepare('INSERT INTO femmes(nom,prenom,age,matricule,activite,stmatrimoniale,telephone,commune,village,nom_regroupement,statut,edition_campagne,capacite,observations,photo,id_user,date_recensement)
                          VALUES(:nomW,:prenomW,:ageW,:matriculeW,:activiteW,:stmatrimonialeW,:telephoneW,:communeW,:villageW,:nom_regroupementW,:statutW,:edition_campagneW,:capaciteW,:observationsW,:picture,:iduser,CURDATE() )');
    ...
    $req->closeCursor();
    header('Location:../dashboard/recensement.php?campagneID='.$_POST['campagneid']);
...
//Fonction de récupération de dernier matricule de la base de données
...
//Fonction de vérification de l'existence du matricule généré dans la base de donnée
